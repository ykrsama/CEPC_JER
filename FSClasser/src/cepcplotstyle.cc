		...
		double yhi  )
{
	...
	for( unsigned int i=0; i<datahist.size(); i++){
		if(datahist[i]->Integral()>0 && dataname[i]){
			if(i==0) leg->AddEntry(datahist[i], dataname[i], "LEP");
			else     leg->AddEntry(datahist[i], dataname[i], "LF");
		}
	}
		...
		)
{
	...
	if(nh>1)FormatMC1(datahist[1]);
	if(nh>2)FormatMC2(datahist[2]);
	if(nh>3)FormatMC3(datahist[3]);
	if(nh>4)FormatMC4(datahist[4]);
	//
	if(nh>1)datahist[0]->SetMaximum(max(datahist[1]->GetMaximum(),datahist[0]->GetMaximum())*h_scale);
	if(nh>2)datahist[0]->SetMaximum(max(datahist[2]->GetMaximum(),datahist[0]->GetMaximum())*h_scale);
	if(nh>3)datahist[0]->SetMaximum(max(datahist[3]->GetMaximum(),datahist[0]->GetMaximum())*h_scale);
	if(nh>4)datahist[0]->SetMaximum(max(datahist[4]->GetMaximum(),datahist[0]->GetMaximum())*h_scale);
	//
	if(logy){
		gPad->SetLogy();	
	...
	//
	if( datahist[0]->Integral()>0) datahist[0]->Draw("E"); 
	else                           datahist[0]->Draw("axis");
	if(nh>1   )
...
{ 
	...
	//
	if( datahist->Integral()>0) datahist->Draw("E"); 
	else                        datahist->Draw("axis");
	if(mc4hist)
