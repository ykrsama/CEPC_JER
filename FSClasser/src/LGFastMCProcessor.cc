...
namespace marlin{
	...
	void LGFastMCProcessor::processEvent( LCEvent * evt ) { 
		...
		for(int i=0; i<mcpCol->getNumberOfElements() ; i++){
			...
			//double ener    = mcp->getEnergy();
			//cout<<mcp->getCharge()<<endl;
			...
			if( mcp->getGeneratorStatus() == 1 && pdgCODE != 12 && pdgCODE != 14 && pdgCODE != 16) { // stable particles only
				if( m_makeplots>0 ) {
	...
	void LGFastMCProcessor::end(){ 
		...
		if( m_makeplots>0 ) { 
			...
			for( int i=0; i<7; i++){
				...
				h_Vertex[i]->Fit("expo", "+");
				sprintf(filename,"figs/Vertex_%s",h_Vertex[i]->GetTitle());
